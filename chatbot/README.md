...
```
Browser (WebSocket ws://host/ws/chat?token=<jwt>)
    │
    ▼
nginx (port 80)  ──────────────────────────  Angular frontend (port 80)
    │
    ▼
...
|---------|-------|-------|-------------|
| `nginx` | `chatbot-nginx:latest` | `80:80` | TLS termination, routing, static hosting |
| `frontend` | `chatbot-frontend:latest` | internal `80` | Angular SPA |
| `backend` | `chatbot-backend:latest` | internal `8000` | FastAPI + uvicorn |
